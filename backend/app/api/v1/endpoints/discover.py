...
async def _prefetch_visible_details(results: List[Dict[str, Any]]) -> None:
    """
    Warm the detail caches for the first cards of a browse page so clicking one
    answers locally. Runs after the response is sent, through the normal cached
    service methods.
    """
    try:
        fetchers = []
        ...
        for item in results:
            ...
            if not item_id:
                continue
            if item_type == "movie" and movie_count < 6:
                fetchers.append(tmdb_service.get_movie(item_id))
                movie_count += 1
            elif item_type in ("show", "tv") and show_count < 6:
                fetchers.append(tmdb_service.get_tv(item_id))
                show_count += 1
            elif item_type == "anime" and anime_count < 3:
                fetchers.append(anilist_service.get_anime(item_id))
                anime_count += 1
        if fetchers:
...
async def get_trending(
    ...
    try:
        ...
        background_tasks.add_task(_prefetch_visible_details, formatted_results)
...
async def get_popular(
    ...
    try:
        ...
        background_tasks.add_task(_prefetch_visible_details, all_results)
...
async def get_by_genre(
    ...
    try:
        ...
        if media_type in ["all", "movie"]:
            movie_data = await tmdb_service.discover_movies(genres=movie_genre_ids, page=page, sort_by=tmdb_sort)
        ...
        if media_type in ["all", "show", "tv"]:
            tv_data = await tmdb_service.discover_tv(genres=tv_genre_ids, page=page, sort_by=tmdb_sort)
        ...
        if media_type in ["all", "anime"]:
            anime_results = await anilist_service.get_by_genre(anilist_genre, page=page, per_page=20)
            for anime in anime_results:
                title = anime.get("title", {})
                anime_title = title.get("english") or title.get("romaji")
