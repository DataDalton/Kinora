...
from app.schemas.permission import (
    Permission, PermissionGroup, PermissionGroupCreate,
    PermissionGroupUpdate, PermissionGroupSimple
)
...
async def require_user_manage_permission(
    ...
    if not hasUsersPermission and not hasGroupsPermission:
        raise HTTPException(
            ...
            detail="You do not have permission to manage users and groups"
        )
    return current_user
...
async def create_permission_group(
    ...
    if existing:
        raise HTTPException(
            ...
            detail="A permission group with this name already exists"
        )
...
async def update_permission_group(
    ...
    if existingGroup["is_system"]:
        if group_data.displayName is not None or group_data.description is not None or group_data.color is not None:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Cannot modify core attributes of system groups"
            )
    ...
    if group_data.permissionNames is not None:
        ...
        await conn.execute(
            "DELETE FROM permission_group_permissions WHERE group_id = $1",
            group_id
        )
