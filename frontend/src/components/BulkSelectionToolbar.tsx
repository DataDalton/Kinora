...
export default function BulkSelectionToolbar({
	...
	const [showSelectByTagDropdown, setShowSelectByTagDropdown] =
		useState(false);
	...
	useEffect(() => {
		const handleClickOutside = (event: MouseEvent) => {
			...
			if (
				selectByTagRef.current &&
				!selectByTagRef.current.contains(event.target as Node)
			) {
				setShowSelectByTagDropdown(false);
			}
		};
						...
						{onSelectByTag && availableTagsFromItems.length > 0 && (
							<div className="relative" ref={selectByTagRef}>
								...
								>
									...
									Select by Tag
									<ChevronDown
										className={`w-4 h-4 transition-transform ${showSelectByTagDropdown ? "rotate-180" : ""}`}
									/>
								</button>
			...
			{showProfileSelector && (
				<div className="fixed inset-0 z-50 flex items-center justify-center">
					...
					<div className="relative bg-background rounded-xl border border-border shadow-2xl w-full max-w-md mx-4 p-6">
						...
						<p className="text-muted-foreground mb-4">
							Select a media profile for {selectedIds.length}{" "}
							item(s)
						</p>
						<div className="max-h-60 overflow-y-auto mb-6 space-y-1">
