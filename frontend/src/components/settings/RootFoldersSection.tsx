...
export default function RootFoldersSection() {
  ...
  const handleMovePriority = async (folderId: number, direction: 'up' | 'down') => {
    ...
    // Set priorities based on target positions (not existing values, which may be identical)
    try {
      await Promise.all([
        updateRootFolder(currentFolder.id, { priority: swapIndex }),
        updateRootFolder(swapFolder.id, { priority: currentIndex }),
      ]);
      queryClient.invalidateQueries({ queryKey: ['root-folders'] });
    } catch {
