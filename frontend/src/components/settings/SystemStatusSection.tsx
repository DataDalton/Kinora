...
function getDownloadClientStatusColor(
...
function DownloadClientStatusIcon({
	status,
}: {
	status: DownloadClientStatusLevel;
}) {
...
function CoreServiceCard({
	...
	return (
		<div
			className={`bg-card border rounded-lg p-4 ${getServiceStatusBg(service.status)}`}
		>
			<div className="flex items-center justify-between mb-3">
				...
				<div
					className={`flex items-center gap-2 ${getServiceStatusColor(service.status)}`}
				>
...
function DownloadClientCard({ client }: { client: DownloadClientStatus }) {
	return (
		<div
			className={`bg-card border rounded-lg p-4 ${getDownloadClientStatusBg(client.status)}`}
		>
			<div className="flex items-center justify-between">
				...
				<div
					className={`flex items-center gap-2 ${getDownloadClientStatusColor(client.status)}`}
				>
...
function ExternalApiCard({
	...
	name: string;
	api: ExternalApiStatus;
}) {
	return (
		<div
			className={`bg-card border rounded-lg p-4 ${getApiStatusBg(api.status)}`}
		>
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-3">
					<div className="p-2 bg-primary/10 rounded-lg">
				...
				<div
					className={`flex items-center gap-2 ${getApiStatusColor(api.status)}`}
				>
					{api.status === "healthy" && (
		...
		<div className="space-y-8 animate-pulse">
			<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
				{[1, 2, 3].map((i) => (
					<div
						key={i}
						className="bg-card border border-border rounded-lg p-4 h-24"
					/>
				))}
			</div>
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
				{[1, 2, 3].map((i) => (
					<div
						key={i}
						className="bg-card border border-border rounded-lg p-4 h-20"
					/>
				))}
			</div>
						...
						<div className="space-y-8">
							...
							<section>
								...
								{systemStatus.downloadClients.length === 0 ? (
									...
									<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
										{systemStatus.downloadClients.map(
											(client) => (
												<DownloadClientCard
													key={client.id}
													client={client}
							...
							<section>
								...
								{Object.keys(systemStatus.externalApis)
									.length === 0 ? (
									<div className="text-center py-8 bg-card border border-border rounded-lg text-muted-foreground">
							...
							<section>
								...
								<CeleryTasksTable
									tasks={systemStatus.celeryTasks}
								/>
							</section>
						</div>
					)}
