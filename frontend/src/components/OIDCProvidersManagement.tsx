...
export default function OIDCProvidersManagement() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["oidc-providers-admin"] });
      setShowModal(false);
      resetForm();
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: Partial<OIDCProvider> }) => {
    ...
    <div>
      ...
      {providers && providers.length > 0 ? (
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-mono text-xs mt-1">{provider.scopes}</p>
                    </div>
                    {provider.button_text && (
                      <div>
                        <span className="text-muted-foreground">Button Text:</span>
                        <p className="text-xs mt-1">{provider.button_text}</p>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="text-center py-12 bg-card border border-border rounded-lg">
          ...
          <p className="text-muted-foreground mb-4">No OIDC providers configured</p>
          <p className="text-sm text-muted-foreground max-w-md mx-auto">
            Add an OIDC provider to enable single sign-on with services like Google,
            Microsoft, Keycloak, or Authentik
          </p>
        </div>
      )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Base URL for OIDC discovery (without /.well-known/openid-configuration)
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, button_icon: e.target.value })
                    }
                    className="w-full px-4 py-2 border border-border bg-background text-foreground rounded-lg focus:ring-2 focus:ring-primary"
                    placeholder="🔐"
                  />
