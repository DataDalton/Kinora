...
import { Tag, X, Plus, ChevronDown, Check, Palette } from "lucide-react";
...
export default function TagsEditor({
	...
	const addTagMutation = useMutation({
		mutationFn: async (tagId: number) => {
			const response = await api.post(
				`/tags/media/${mediaType}/${mediaId}/add/${tagId}`,
			);
			return response.data as TagItem[];
		},
		onSuccess: (newTags) => {
	...
	const removeTagMutation = useMutation({
		mutationFn: async (tagId: number) => {
			const response = await api.delete(
				`/tags/media/${mediaType}/${mediaId}/remove/${tagId}`,
			);
			return response.data as TagItem[];
		},
		onSuccess: (newTags) => {
	...
	const availableTags =
		allTags?.filter((t) => !currentTagIds.has(t.id)) || [];
										...
										{defaultColors.map((color) => (
											<button
												...
												className={`w-6 h-6 rounded-full transition-transform cursor-pointer ${
													newTagColor === color
														? "scale-110 ring-2 ring-offset-2 ring-offset-background ring-primary"
														: ""
												}`}
												style={{
