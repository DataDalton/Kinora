...
export default function ForwardAuthSettings() {
	...
	const handleSave = async () => {
		try {
			...
			if (value) {
				await api.put("/settings/forward_auth_trusted_proxies", {
					value,
				});
			} else {
				try {
		...
		<div className="bg-card border border-border rounded-lg p-6">
			<div className="flex items-center gap-3 mb-4">
				...
				<div>
					...
					<p className="text-sm text-muted-foreground">
						Configure trusted proxy IPs for Authelia/Authentik
			...
			<div className="mb-6 p-4 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900 rounded-lg">
				<div className="flex gap-2">
					...
					<div className="text-sm text-blue-900 dark:text-blue-100">
						...
						<p className="mb-2">
							When Authelia or Authentik is configured as a
							reverse proxy in front of Kinora, authentication
							headers are automatically forwarded. Kinora will
							only accept these headers from trusted proxy IP
							addresses.
						</p>
						<p className="font-semibold mb-1">
							Default Trusted Ranges (used when list is empty):
						</p>
				...
				<div>
					...
					{trustedProxies.length > 0 ? (
						...
						<div className="p-4 bg-muted/30 border border-border rounded-lg text-center">
							<p className="text-sm text-muted-foreground">
								No custom proxies configured. Using default
								private IP ranges.
							</p>
						</div>
					)}
			...
			<div className="mt-6 p-4 bg-muted/30 border border-border rounded-lg">
				<p className="text-xs text-muted-foreground">
					<strong>Security Note:</strong> Only add IP addresses that
					you trust. These IPs can authenticate users to your system
					via headers. Common values include your Docker network
					gateway (e.g., 172.18.0.1) or your reverse proxy's IP
					address.
				</p>
			</div>
		</div>
