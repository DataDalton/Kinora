...
const permissionCategories = {
  system: ['system.admin', 'system.settings', 'system.users', 'system.groups', 'system.logs', 'requests.view', 'requests.manage'],
  users: ['users.password.self', 'users.password.reset'],
  movies: ['movies.view', 'movies.manage', 'movies.request', 'movies.approve', 'movies.download'],
  shows: ['shows.view', 'shows.manage', 'shows.request', 'shows.approve', 'shows.download'],
  anime: ['anime.view', 'anime.manage', 'anime.request', 'anime.approve', 'anime.download'],
  music: ['music.view', 'music.manage', 'music.request', 'music.approve', 'music.download'],
};
...
const permissionImplications: Record<string, string[]> = {
  ...
  'system.admin': [
    'system.settings', 'system.users', 'system.groups', 'system.logs',
    'requests.view', 'requests.manage',
    'users.password.self', 'users.password.reset',
    'movies.view', 'movies.manage', 'movies.request', 'movies.approve', 'movies.download',
    'shows.view', 'shows.manage', 'shows.request', 'shows.approve', 'shows.download',
    'anime.view', 'anime.manage', 'anime.request', 'anime.approve', 'anime.download',
    'music.view', 'music.manage', 'music.request', 'music.approve', 'music.download',
  ],
  ...
  'movies.manage': ['movies.view', 'movies.request', 'movies.approve', 'movies.download'],
  'shows.manage': ['shows.view', 'shows.request', 'shows.approve', 'shows.download'],
  'anime.manage': ['anime.view', 'anime.request', 'anime.approve', 'anime.download'],
  'music.manage': ['music.view', 'music.request', 'music.approve', 'music.download'],
...
export default function SettingsPage() {
  ...
  const [newUserData, setNewUserData] = useState<{ username: string; password: string; groupIds: number[]; isActive: boolean }>({ username: '', password: '', groupIds: [], isActive: true });
  const [editUserData, setEditUserData] = useState<{ username: string; groupIds: number[]; isActive: boolean }>({ username: '', groupIds: [], isActive: true });
  ...
  const [newGroupData, setNewGroupData] = useState<{ name: string; displayName: string; description: string; color: string; permissionNames: string[] }>({ name: '', displayName: '', description: '', color: predefinedColors[0], permissionNames: [] });
  const [editGroupData, setEditGroupData] = useState<{ displayName: string; description: string; color: string; permissionNames: string[] }>({ displayName: '', description: '', color: '', permissionNames: [] });
  ...
  const { data: users } = useQuery<AppUser[]>({
    ...
    queryFn: async () => {
      ...
      return response.data.map((user: Record<string, unknown>) => ({
        ...user,
        groups: ((user.groups as Array<Record<string, unknown>>) || []).map((g: Record<string, unknown>) => ({
          id: g.id,
          name: g.name,
  ...
  const createUserMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setNewUserData({ username: '', password: '', groupIds: [], isActive: true });
  ...
  const createGroupMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setNewGroupData({ name: '', displayName: '', description: '', color: predefinedColors[0], permissionNames: [] });
  ...
  const updateGroupMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: PermissionGroupUpdate }) => {
  ...
  const handleDeleteGroup = (group: PermissionGroup) => {
    ...
    const affectedUsers = users?.filter(u => u.groups?.some(g => g.id === group.id)) || [];
    setConfirmDialog({
      isOpen: true,
  ...
  const togglePermission = (permissionName: string, currentPermissions: string[], setPermissions: (perms: string[]) => void) => {
    if (currentPermissions.includes(permissionName)) {
      ...
      const implied = getImpliedPermissions(permissionName);
      const newPermissions = new Set([...currentPermissions, permissionName, ...implied]);
  ...
  const categories = [
    {
      ...
      sections: [
        ...
        { id: 'permission-groups' as SettingsSection, name: 'Permission Groups', count: permissionGroups?.length || 0 },
          ...
          {selectedSection === 'permission-groups' && (
            <div>
              ...
              <div className="p-8">
                <div className="max-w-6xl mx-auto">
                  <div className="mb-6 flex justify-between items-center">
                    <h2 className="text-xl font-bold">Permission Groups</h2>
                    <button
                          ...
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                <span>{group.permissions?.length || 0} permissions</span>
                                <span>|</span>
        ...
        <div className="fixed inset-0 backdrop-blur-sm bg-background/50 z-50 flex items-center justify-center p-4">
          <div className="bg-background rounded-lg max-w-lg w-full border border-border shadow-2xl p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-2">Permission Groups</label>
                <div className="space-y-1 max-h-48 overflow-y-auto border border-border rounded-lg p-2">
                  {permissionGroups && permissionGroups.length > 0 ? (
                    permissionGroups.map((group) => {
                      const isSelected = newUserData.groupIds.includes(group.id);
                      return (
                        <button
                          key={group.id}
                          type="button"
                          onClick={() => {
                            if (isSelected) {
                              setNewUserData({ ...newUserData, groupIds: newUserData.groupIds.filter(id => id !== group.id) });
                            } else {
                              setNewUserData({ ...newUserData, groupIds: [...newUserData.groupIds, group.id] });
                            }
                          }}
                    ...
                    <p className="text-sm text-muted-foreground p-2">No permission groups available</p>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-1 max-h-48 overflow-y-auto border border-border rounded-lg p-2">
                  {permissionGroups && permissionGroups.length > 0 ? (
                    permissionGroups.map((group) => {
                      const isSelected = editUserData.groupIds.includes(group.id);
                      return (
                        <button
                          key={group.id}
                          type="button"
                          onClick={() => {
                            if (isSelected) {
                              setEditUserData({ ...editUserData, groupIds: editUserData.groupIds.filter(id => id !== group.id) });
                            } else {
                              setEditUserData({ ...editUserData, groupIds: [...editUserData.groupIds, group.id] });
                            }
                          }}
                          className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition cursor-pointer ${
                            isSelected
        ...
        <div className="fixed inset-0 backdrop-blur-sm bg-background/50 z-50 flex items-center justify-center p-4">
          <div className="bg-background rounded-lg max-w-5xl w-full border border-border shadow-2xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-6 border-b border-border">
              <h2 className="text-2xl font-bold">Create Permission Group</h2>
            </div>
            <div className="flex-1 flex overflow-hidden">
              ...
              <div className="w-80 shrink-0 p-6 border-r border-border overflow-y-auto">
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium mb-1">Name (identifier)</label>
                    <input
                      type="text"
                      value={newGroupData.name}
                      onChange={(e) => setNewGroupData({ ...newGroupData, name: e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, '_') })}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewGroupData({ ...newGroupData, displayName: e.target.value })}
                      className="w-full px-3 py-2 border border-border bg-background text-foreground rounded-lg focus:ring-2 focus:ring-primary text-sm"
                    ...
                    <div className="flex flex-wrap gap-2">
                      {predefinedColors.map((color) => (
                        <button
                          key={color}
                          type="button"
                          onClick={() => setNewGroupData({ ...newGroupData, color })}
                  ...
                  <div className="pt-2">
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <span
                        ...
                        style={{ backgroundColor: newGroupData.color || predefinedColors[0] }}
                      />
                      <span className="font-medium text-foreground">{newGroupData.displayName || 'Preview'}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 p-6 overflow-y-auto">
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {newGroupData.permissionNames.length} selected
                  </span>
                </div>
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            const allSelected = categoryPermNames.every(p => newGroupData.permissionNames.includes(p));
                            if (allSelected) {
                              setNewGroupData({ ...newGroupData, permissionNames: newGroupData.permissionNames.filter(p => !categoryPermNames.includes(p)) });
                            } else {
                              const newPerms = [...new Set([...newGroupData.permissionNames, ...categoryPermNames])];
                              setNewGroupData({ ...newGroupData, permissionNames: newPerms });
                            }
                          }}
                          className="text-xs text-primary hover:underline cursor-pointer"
                        >
                          {perms.every(p => newGroupData.permissionNames.includes(p.name)) ? 'Deselect all' : 'Select all'}
                        </button>
                      </div>
                      <div className="grid grid-cols-2 lg:grid-cols-3 gap-2">
                        {perms.map((perm) => {
                          const isChecked = newGroupData.permissionNames.includes(perm.name);
                          return (
                            <button
                              key={perm.name}
                              type="button"
                              onClick={() => togglePermission(perm.name, newGroupData.permissionNames, (perms) => setNewGroupData({ ...newGroupData, permissionNames: perms }))}
                            ...
                            >
                              ...
                              <span className={isChecked ? 'font-medium' : ''}>{perm.displayName}</span>
                            </button>
                          );
                        })}
                      </div>
            ...
            <div className="p-6 border-t border-border flex gap-3">
              <button
                ...
                disabled={createGroupMutation.isPending || !newGroupData.name || !newGroupData.displayName}
                className="flex-1 px-6 py-3 bg-primary text-primary-foreground rounded-lg hover:opacity-90 disabled:opacity-50 cursor-pointer transition"
              >
          ...
          <div className="bg-background rounded-lg max-w-5xl w-full border border-border shadow-2xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-6 border-b border-border">
              <div className="flex items-center gap-3">
                <h2 className="text-2xl font-bold">Edit Permission Group</h2>
                {selectedGroup.isSystem && (
                  <span className="px-2 py-1 bg-muted text-muted-foreground text-sm rounded-full">System</span>
                )}
              </div>
            </div>
            <div className="flex-1 flex overflow-hidden">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEditGroupData({ ...editGroupData, displayName: e.target.value })}
                      className="w-full px-3 py-2 border border-border bg-background text-foreground rounded-lg focus:ring-2 focus:ring-primary text-sm"
                      placeholder="Group Name"
                    />
                  </div>
                  ...
                  <div className="pt-2">
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <span
                        className="w-3 h-3 rounded-full"
                        style={{ backgroundColor: editGroupData.color || predefinedColors[0] }}
                      />
                      <span className="font-medium text-foreground">{editGroupData.displayName || 'Preview'}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 p-6 overflow-y-auto">
                <div className="mb-4 flex items-center justify-between">
                  <h3 className="text-lg font-semibold">Permissions</h3>
                  <span className="text-sm text-muted-foreground">
                    {editGroupData.permissionNames.length} selected
                  </span>
                </div>
                <div className="space-y-6">
                  {Object.entries(permissionsByCategory).map(([category, perms]) => (
                    <div key={category}>
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <button
                          ...
                          onClick={() => {
                            const categoryPermNames = perms.map(p => p.name);
                            const allSelected = categoryPermNames.every(p => editGroupData.permissionNames.includes(p));
                            if (allSelected) {
                              setEditGroupData({ ...editGroupData, permissionNames: editGroupData.permissionNames.filter(p => !categoryPermNames.includes(p)) });
                            } else {
                              const newPerms = [...new Set([...editGroupData.permissionNames, ...categoryPermNames])];
                              setEditGroupData({ ...editGroupData, permissionNames: newPerms });
                            }
                          }}
                          className="text-xs text-primary hover:underline cursor-pointer"
                        >
                          {perms.every(p => editGroupData.permissionNames.includes(p.name)) ? 'Deselect all' : 'Select all'}
                        </button>
                      </div>
                      <div className="grid grid-cols-2 lg:grid-cols-3 gap-2">
                        {perms.map((perm) => {
                          const isChecked = editGroupData.permissionNames.includes(perm.name);
                          return (
                            <button
                              key={perm.name}
                              type="button"
                              onClick={() => togglePermission(perm.name, editGroupData.permissionNames, (perms) => setEditGroupData({ ...editGroupData, permissionNames: perms }))}
                              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-left transition cursor-pointer ${
                                isChecked
                                  ? 'bg-primary/10 ring-1 ring-primary'
                                  : 'bg-muted/50 hover:bg-muted'
            ...
            <div className="p-6 border-t border-border flex gap-3">
              <button
                onClick={() => updateGroupMutation.mutate({ id: selectedGroup.id, data: editGroupData })}
                disabled={updateGroupMutation.isPending || !editGroupData.displayName}
                className="flex-1 px-6 py-3 bg-primary text-primary-foreground rounded-lg hover:opacity-90 disabled:opacity-50 cursor-pointer transition"
              >
