...
export default function MovieDetailPage() {
	...
	const refreshMetadataMutation = useMutation({
		mutationFn: async () => {
			const response = await api.post(
				`/movies/${movieId}/refresh-metadata`,
			);
			return response.data;
		},
		onSuccess: () => {
	...
	const deleteMovieMutation = useMutation({
		mutationFn: async (deleteFiles: boolean) => {
			const response = await api.delete(
				`/movies/${movieId}/delete?delete_files=${deleteFiles}`,
			);
			return response.data;
		},
		onSuccess: () => {
	...
	const getStatusBadge = (status: string, hasFile: boolean) => {
		...
		return (
			<span className="px-3 py-1 text-sm rounded bg-gray-500/20 text-gray-400 border border-gray-500/50 font-medium capitalize">
				{status}
			</span>
		);
	};
		...
		<div className="min-h-screen">
			...
			<div className="relative z-10">
				<PageHeader
					...
					description={
						movie.tagline ||
						`${movie.release_date ? new Date(movie.release_date).getFullYear() : "Unknown"} • ${formatRuntime(movie.runtime)}`
					}
					gradientFrom="blue-600/10"
					gradientVia="purple-600/10"
					gradientTo="pink-600/10"
				...
				<div className="container mx-auto px-6 py-8">
					<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
								...
								<div className="p-6 space-y-4">
									...
									<div className="space-y-2">
										...
										<div className="flex gap-2">
											...
											<button
												onClick={() =>
													rescanFilesMutation.mutate()
												}
												disabled={
													rescanFilesMutation.isPending
												}
												className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-card text-foreground border-2 border-border rounded-lg hover:bg-accent transition font-medium cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
												title="Rescan files on disk"
											>
												<ScanLine
													className={`w-5 h-5 ${rescanFilesMutation.isPending ? "animate-spin" : ""}`}
												/>
												Rescan
											</button>
										</div>
						...
						<div className="lg:col-span-2 space-y-6">
							...
							{/* Tags */}
							<TagsEditor mediaType="movie" mediaId={movie.id} />
