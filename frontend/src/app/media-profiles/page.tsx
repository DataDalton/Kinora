...
export default function MediaProfilesPage() {
	...
	const [editingProfile, setEditingProfile] = useState<MediaProfile | null>(
		null,
	);
			...
			const response = await api.post("/media-profiles", {
				...
				uploaders: data.uploader_filter
					? data.uploader_filter
							...
							.filter(Boolean)
					: [],
				release_groups: data.release_group_filter
					? data.release_group_filter
							.split(",")
							.map((s) => s.trim())
							.filter(Boolean)
					: [],
	...
	const handleEdit = (profile: MediaProfile) => {
		...
		setFormData({
			...
			search_sort_preference:
				(profile.search_sort_preference as
					| "weighted"
					| "seeders"
					| "size"
					| "date") || "weighted",
			season_pack_preference:
				(profile.season_pack_preference as
					| "prefer"
					| "only"
					| "avoid") || "prefer",
			...
			movie_folder_format:
				profile.movie_folder_format || defaults.movie_folder_format,
			show_naming_format:
				profile.show_naming_format || defaults.show_naming_format,
			show_folder_format:
				profile.show_folder_format || defaults.show_folder_format,
			anime_naming_format:
				profile.anime_naming_format || defaults.anime_naming_format,
			anime_folder_format:
				profile.anime_folder_format || defaults.anime_folder_format,
			// Anime options
			anime_subtitle_preference:
				(profile.anime_subtitle_preference as
					| "softsub"
					| "hardsub"
					| "dual_audio") || "softsub",
			...
			// Music settings
			music_artist_folder_format:
				profile.music_artist_folder_format ||
				defaults.music_artist_folder_format,
			music_album_folder_format:
				profile.music_album_folder_format ||
				defaults.music_album_folder_format,
			music_track_naming_format:
				profile.music_track_naming_format ||
				defaults.music_track_naming_format,
			...
			validation_mode:
				(profile.validation_mode as "blocklist" | "allowlist") ||
				"allowlist",
			forbidden_extensions:
				profile.forbidden_extensions || defaults.forbidden_extensions,
			validation_failure_action:
				(profile.validation_failure_action as
					| "delete"
					| "pause_notify"
					| "quarantine") || "pause_notify",
			movie_allowed_extensions:
				profile.movie_allowed_extensions ||
				defaults.movie_allowed_extensions,
			show_allowed_extensions:
				profile.show_allowed_extensions ||
				defaults.show_allowed_extensions,
			anime_allowed_extensions:
				profile.anime_allowed_extensions ||
				defaults.anime_allowed_extensions,
	...
	const validateForm = () => {
		...
		if (
			formData.movie_indexers.length > 0 &&
			formData.movie_resolutions.length === 0
		) {
			errors.push(
				"Movie resolutions are required when movie indexers are selected",
			);
		}
		if (
			formData.show_indexers.length > 0 &&
			formData.show_resolutions.length === 0
		) {
			errors.push(
				"TV show resolutions are required when show indexers are selected",
			);
		}
		if (
			formData.anime_indexers.length > 0 &&
			formData.anime_resolutions.length === 0
		) {
			errors.push(
				"Anime resolutions are required when anime indexers are selected",
			);
		}
		if (
			formData.music_indexers.length > 0 &&
		...
		if (
			formData.movie_indexers.length > 0 &&
			!formData.movie_naming_format.trim()
		) {
			errors.push(
				"Movie naming format is required when movie indexers are selected",
			);
		}
		if (
			formData.show_indexers.length > 0 &&
			!formData.show_naming_format.trim()
		) {
			errors.push(
				"TV show naming format is required when show indexers are selected",
			);
		}
		if (
			formData.anime_indexers.length > 0 &&
			!formData.anime_naming_format.trim()
		) {
			errors.push(
				"Anime naming format is required when anime indexers are selected",
			);
		}
		if (
			formData.music_indexers.length > 0 &&
			!formData.music_track_naming_format.trim()
		) {
			errors.push(
				"Music track naming format is required when music indexers are selected",
			);
		}
		...
		if (formData.validation_enabled) {
			if (formData.validation_mode === "allowlist") {
				if (
					formData.movie_indexers.length > 0 &&
					formData.movie_allowed_extensions.length === 0
				) {
					errors.push(
						"Movie allowed extensions are required when validation is enabled",
					);
				}
				if (
					formData.show_indexers.length > 0 &&
					formData.show_allowed_extensions.length === 0
				) {
					errors.push(
						"TV show allowed extensions are required when validation is enabled",
					);
				}
				if (
					formData.anime_indexers.length > 0 &&
					formData.anime_allowed_extensions.length === 0
				) {
					errors.push(
						"Anime allowed extensions are required when validation is enabled",
					);
				}
				if (
					formData.music_indexers.length > 0 &&
					formData.music_allowed_extensions.length === 0
				) {
					errors.push(
						"Music allowed extensions are required when validation is enabled",
					);
				}
			} else if (formData.validation_mode === "blocklist") {
				if (formData.forbidden_extensions.length === 0) {
					errors.push(
						"Forbidden extensions are required when blocklist mode is enabled",
					);
				}
			}
		}
	...
	const handleDelete = (id: number) => {
		setConfirmDialog({
			...
			message:
				"Are you sure you want to delete this media profile? This action cannot be undone.",
			onConfirm: () => {
			...
			case "profile":
				return (
					<ProfileGroup
						{...sectionProps}
						activeTab={
							activeTab as "general" | "languages" | "validation"
						}
					/>
				);
			case "movies":
				return (
					<MoviesGroup
						{...sectionProps}
						activeTab={
							activeTab as "indexers" | "quality" | "naming"
						}
					/>
				);
			case "tvshows":
			...
			case "search":
				return (
					<SearchGroup
						{...sectionProps}
						activeTab={
							activeTab as "sorting" | "filters" | "timing"
						}
					/>
				);
			case "fileoutput":
	...
	const getProfileSummary = (profile: MediaProfile) => {
		...
		if (profile.movie_resolutions && profile.movie_resolutions.length > 0) {
			summaryItems.push({
				label: "Movie Quality",
				value:
					profile.movie_resolutions.slice(0, 3).join(", ") +
					(profile.movie_resolutions.length > 3 ? "..." : ""),
			});
		}
		...
		if (profile.show_resolutions && profile.show_resolutions.length > 0) {
			summaryItems.push({
				label: "TV Quality",
				value:
					profile.show_resolutions.slice(0, 3).join(", ") +
					(profile.show_resolutions.length > 3 ? "..." : ""),
			});
		}
		...
		if (profile.anime_resolutions && profile.anime_resolutions.length > 0) {
			summaryItems.push({
				label: "Anime Quality",
				value:
					profile.anime_resolutions.slice(0, 3).join(", ") +
					(profile.anime_resolutions.length > 3 ? "..." : ""),
			});
		}
		...
		// Indexers
		const indexerCount =
			(profile.movie_indexers?.length || 0) +
				...
				{isLoading ? (
					...
					<div className="flex flex-col items-center justify-center py-24">
						<div className="text-center max-w-md">
							...
							<h2 className="text-2xl font-bold mb-3">
								No Media Profiles Yet
							</h2>
							<p className="text-muted-foreground mb-8">
								Create your first media profile to define
								quality preferences, indexer settings, naming
								formats, and more for each media type.
							</p>
							<button
								onClick={() => setShowForm(true)}
					...
					<div className="grid gap-6">
						{profiles?.map((profile: MediaProfile) => (
							<div
								key={profile.id}
								className="bg-card text-card-foreground rounded-xl shadow-md hover:shadow-lg transition-shadow p-6 border border-border"
							>
