			...
			<div className="space-y-6">
				<div className="bg-blue-500/20 border border-blue-500/30 rounded-lg p-4 mb-4">
					...
					<p className="text-xs text-muted-foreground">
						Choose how to sort and score search results. Weighted
						scoring combines multiple factors for the best overall
						result.
					</p>
				</div>
				...
				<div>
					...
					<div className="grid grid-cols-2 md:grid-cols-4 gap-2">
						...
						<button
							...
							onClick={() =>
								setFormData({
									...formData,
									search_sort_preference: "seeders",
								})
							}
							className={`px-4 py-2.5 rounded-lg font-medium transition-all cursor-pointer ${
					...
					<p className="text-xs text-muted-foreground mt-2">
						{formData.search_sort_preference === "weighted" &&
							"Weighted score combining seeders, size, and recency for best overall result"}
						{formData.search_sort_preference === "seeders" &&
							"Prioritize torrents with the most seeders for faster downloads"}
						{formData.search_sort_preference === "size" &&
							"Sort by file size (preferring sizes within your min/max range)"}
						{formData.search_sort_preference === "date" &&
							"Prioritize the most recently uploaded torrents"}
					</p>
					...
					{formData.search_sort_preference === "weighted" && (
						<div className="mt-4 p-4 bg-muted/50 rounded-lg border border-border space-y-4">
							...
							<p className="text-xs text-muted-foreground">
								Adjust how much each factor influences the
								overall score (must total 100%)
							</p>
										...
										onChange={(e) => {
											...
											const sizeRatio =
												formData.size_weight /
													(formData.size_weight +
														formData.recency_weight) ||
												0.5;
											const sizeWeight = Math.round(
												remaining * sizeRatio,
											);
											const recencyWeight =
												remaining - sizeWeight;
											setFormData({
												...formData,
											...
											const seederRatio =
												formData.seeder_weight /
													(formData.seeder_weight +
														formData.recency_weight) ||
												0.5;
											const seederWeight = Math.round(
												remaining * seederRatio,
											);
											const recencyWeight =
												remaining - seederWeight;
											setFormData({
												...formData,
												size_weight: sizeWeight,
												seeder_weight: seederWeight,
		...
		return (
			<div className="space-y-6">
				<div className="bg-blue-500/20 border border-blue-500/30 rounded-lg p-4 mb-4">
					...
					<p className="text-xs text-muted-foreground">
						Filter search results by uploader, release group, or
						custom regex patterns. These filters apply before
						quality scoring.
					</p>
				</div>
					...
					<div>
						...
						<input
							...
							onChange={(e) =>
								setFormData({
									...formData,
									uploader_filter: e.target.value,
								})
							}
						...
						<p className="text-xs text-muted-foreground mt-1">
							Only include releases from these uploaders
						</p>
					</div>
					...
					<div>
						...
						<input
							...
							onChange={(e) =>
								setFormData({
									...formData,
									release_group_filter: e.target.value,
								})
							}
							className="w-full px-4 py-2.5 border-input bg-background text-foreground border rounded-lg focus:ring-2 focus:ring-primary"
						...
						/>
						<p className="text-xs text-muted-foreground mt-1">
							Only include releases from these groups
						</p>
					</div>
				</div>
	...
	if (activeTab === "timing") {
		return (
			<div className="space-y-6">
				<div className="bg-blue-500/20 border border-blue-500/30 rounded-lg p-4 mb-4">
					<h4 className="font-semibold text-sm mb-2">
						Search Timing Settings
					</h4>
					<p className="text-xs text-muted-foreground">
						Configure timeout, retry behavior, and result limits for
						search operations.
					</p>
				</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
					<div>
						...
						/>
						<p className="text-xs text-muted-foreground mt-1">
							How long to wait for indexer response
						</p>
					</div>
					...
					<div>
						...
						/>
						<p className="text-xs text-muted-foreground mt-1">
							Maximum search results per indexer
						</p>
					</div>
					...
					<div>
						...
						/>
						<p className="text-xs text-muted-foreground mt-1">
							Reject releases with fewer seeders than this
						</p>
					</div>
				</div>
			</div>
