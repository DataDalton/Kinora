		...
		<div className="space-y-6">
			<div className="bg-purple-500/20 border border-purple-500/30 rounded-lg p-4 mb-4">
				...
				<p className="text-xs text-muted-foreground">
					Configure anime torrent preferences including subtitle
					types, dub vs sub, and indexers.
				</p>
			</div>
			...
			<div>
							...
							{LANGUAGES.filter(
								...
								<button
									...
									onClick={() => {
										setFormData({
											...formData,
											anime_audio_language: lang.code,
										});
										setAnimeAudioLanguageSearch("");
									}}
				...
				{formData.anime_audio_language && (
					<div className="mt-2 flex items-center gap-2 px-3 py-1.5 bg-primary/20 border-2 border-primary rounded-lg text-sm font-medium w-fit">
						<span>
							{LANGUAGES.find(
								(l) => l.code === formData.anime_audio_language,
				...
				<p className="text-xs text-muted-foreground mt-2">
					Original audio language (Japanese for anime, Chinese for
					donghua, Korean for aeni, etc.)
				</p>
			</div>
			...
			<div>
				...
				<div className="relative">
					<input
						...
						onChange={(e) =>
							setAnimeSubtitleLanguageSearch(e.target.value)
						}
						onFocus={() =>
							...
							{LANGUAGES.filter(
								...
								<button
									...
									onClick={() => {
										setFormData({
											...formData,
											anime_subtitle_language: lang.code,
										});
										setAnimeSubtitleLanguageSearch("");
									}}
									className="w-full px-4 py-2 text-left hover:bg-muted transition-colors cursor-pointer text-sm"
				...
				{formData.anime_subtitle_language && (
					<div className="mt-2 flex items-center gap-2 px-3 py-1.5 bg-primary/20 border-2 border-primary rounded-lg text-sm font-medium w-fit">
						<span>
							{LANGUAGES.find(
								(l) =>
									l.code === formData.anime_subtitle_language,
			...
			<div>
				<label className="flex items-center cursor-pointer">
					<input
						...
						onChange={(e) =>
							setFormData({
								...formData,
								anime_allow_hardsub: e.target.checked,
							})
						}
				...
				<p className="text-xs text-muted-foreground mt-1 ml-6">
					Allow torrents with burned-in subtitles (hardsubs) even if
					softsubs are preferred
				</p>
			</div>
			...
			<div>
				<label className="flex items-center cursor-pointer">
					<input
						...
						onChange={(e) =>
							setFormData({
								...formData,
								anime_prefer_dual_audio: e.target.checked,
							})
						}
						className="w-4 h-4 rounded border-border bg-background text-primary focus:ring-2 focus:ring-ring cursor-pointer"
					/>
				...
				<p className="text-xs text-muted-foreground mt-1 ml-6">
					Prioritize releases with both Japanese and English audio
					tracks
				</p>
			</div>
		</div>
