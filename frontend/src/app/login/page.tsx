...
export default function LoginPage() {
	...
	const handleWebAuthnAuth = async () => {
		...
		try {
			...
			if (!credential) {
				throw new Error(
					"WebAuthn authentication was cancelled or no credential found.",
				);
			}
			...
			setError(
				err.response?.data?.detail || "WebAuthn authentication failed.",
			);
			setErrorTrigger((prev) => prev + 1);
		} finally {
			setLoading(false);
					...
					{!requires2FA ? (
						...
						<div className="space-y-6">
							...
							<p className="text-sm text-muted-foreground text-center">
								Please verify your identity using one of the
								following methods:
							</p>
