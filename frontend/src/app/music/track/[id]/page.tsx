...
export default function TrackDetailPage() {
	...
	const refreshMetadataMutation = useMutation({
		mutationFn: async () => {
			const response = await api.post(
				`/music/tracks/${trackId}/refresh-metadata`,
			);
	...
	const deleteTrackMutation = useMutation({
		mutationFn: async (deleteFiles: boolean) => {
			const response = await api.delete(
				`/music/tracks/${trackId}/delete?delete_files=${deleteFiles}`,
			);
			return response.data;
		},
		onSuccess: () => {
	...
	const searchDownloadMutation = useMutation({
		mutationFn: async () => {
			const response = await api.post(
				`/music/tracks/${trackId}/search-download`,
			);
			return response.data;
		},
		onSuccess: () => {
	...
	const backLink = track.album_id
		? `/music/album/${track.album_id}`
		: "/music";
	const backText = track.album_id
		? `Back to ${track.album_title}`
		: "Back to Music";
			...
			<div className="container mx-auto px-6 py-8">
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
					...
					<div className="lg:col-span-2 space-y-6">
						...
						{/* Tags */}
						<TagsEditor mediaType="track" mediaId={track.id} />
